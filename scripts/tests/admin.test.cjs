...
assert.deepEqual(resourceCategories(schema, 'enterprise').map(c => c.label), ['人才', '技术', '专利', 'MAH', '数据', '服务机构']);
...
assert.ok(resourceFilters(schema, 'enterprise', 'mah').find(f => f.field === 'approvalStatus').options.some(o => o.value === '可委托生产'));
...
assert.deepEqual(resourceFilters(schema, 'enterprise', 'technology').find(f => f.field === 'industries').storage, 'root');
